...
void GetLocation(const FunctionCallbackInfo<Value>& args) {
    ...
    if(!enableCoreLocation()) {
        isolate->ThrowException(Exception::TypeError(String::NewFromUtf8(isolate, "ENOLOCATIONSERVICES").ToLocalChecked()));
        return;
    }
    ...
    if(!getCoreLocationPosition(&lat, &lng, &alt, &horizontalAcc, &verticalAcc, &error)) {
        switch(error) {
        case kCLErrorDenied:
            isolate->ThrowException(Exception::TypeError(String::NewFromUtf8(isolate, "ELOCATIONDENIED").ToLocalChecked()));
            return;
        case kCLErrorLocationUnknown:
            isolate->ThrowException(Exception::TypeError(String::NewFromUtf8(isolate, "ELOCATIONUNKNOWN").ToLocalChecked()));
            return;
        default:
            isolate->ThrowException(Exception::TypeError(String::NewFromUtf8(isolate, "EGETLOCATIONFAILED").ToLocalChecked()));
            return;
        }
    }

    Local<Object> obj = Object::New(isolate);
    obj->Set(isolate->GetCurrentContext(), String::NewFromUtf8(isolate, "lat").ToLocalChecked(), v8::Number::New(isolate, static_cast<double>(lat)));
    obj->Set(isolate->GetCurrentContext(), String::NewFromUtf8(isolate, "lng").ToLocalChecked(), v8::Number::New(isolate, static_cast<double>(lng)));
    obj->Set(isolate->GetCurrentContext(), String::NewFromUtf8(isolate, "altitude").ToLocalChecked(), v8::Number::New(isolate, static_cast<double>(alt)));
    obj->Set(isolate->GetCurrentContext(), String::NewFromUtf8(isolate, "horizontalAccuracy").ToLocalChecked(), v8::Number::New(isolate, static_cast<double>(horizontalAcc)));
    obj->Set(isolate->GetCurrentContext(), String::NewFromUtf8(isolate, "verticalAccuracy").ToLocalChecked(), v8::Number::New(isolate, static_cast<double>(verticalAcc)));
